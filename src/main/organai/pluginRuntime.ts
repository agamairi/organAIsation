...
export class PluginRuntime {
  ...
  install(id: string, stagePath: string): { ok: boolean; error?: string } {
    ...
    try { const source = resolve(stagePath); const manifest = join(source, 'plugin.json'); if (!existsSync(manifest)) return { ok: false, error: 'staged plugin files are missing' }; const destination = join(this.pluginsDir, id); rmSync(destination, { recursive: true, force: true }); cpSync(source, destination, { recursive: true, filter: (candidate) => !candidate.includes(`${sep}node_modules${sep}`) }); record.state = 'installed'; record.installedAt = new Date().toISOString(); this.persist(); return { ok: true }; } catch (error) { return { ok: false, error: error instanceof Error ? error.message : String(error) }; }
  }
