...
export class MemoryV2Service {
  ...
  retrieve(query: MemoryQuery): MemoryHit[] {
    ...
    const clauses = ['contradicted_by IS NULL']; const params: unknown[] = [];
    const visibility: string[] = [];
    if (query.agentId) { visibility.push('(scope = ? AND owner_id = ?)'); params.push('agent', query.agentId); }
    if (query.includeFounder !== false) visibility.push("scope = 'founder'"); if (query.includeCompany !== false) visibility.push("scope = 'company'"); if (query.projectId) { visibility.push('(scope = ? AND project_id = ?)'); params.push('project', query.projectId); }
    if (!visibility.length) return [];
    clauses.push(`(${visibility.join(' OR ')})`);
    const like = words.map(() => 'content LIKE ?').join(' OR '); params.push(...words.map((word) => `%${word}%`));
    const rows = this.db ? this.db.prepare(`SELECT * FROM memories WHERE ${clauses.join(' AND ')} AND (${like}) ORDER BY pinned DESC, updated_at DESC LIMIT ?`).all(...params, Math.min(Math.max(query.limit ?? 8, 1), 30)).map((row) => this.entry(row as Record<string, unknown>)) : this.fallback.memories.filter((entry) => !entry.contradictedBy && ((query.agentId && entry.scope === 'agent' && entry.ownerId === query.agentId) || (query.includeFounder !== false && entry.scope === 'founder') || (query.includeCompany !== false && entry.scope === 'company') || (query.projectId && entry.scope === 'project' && entry.projectId === query.projectId)) && words.some((word) => entry.content.toLowerCase().includes(word.toLowerCase()))).sort((a, b) => Number(b.pinned) - Number(a.pinned) || b.updatedAt.localeCompare(a.updatedAt)).slice(0, Math.min(Math.max(query.limit ?? 8, 1), 30));
  ...
  supersede(oldId: string, replacement: Omit<MemoryEntry, 'id' | 'createdAt' | 'updatedAt'>): { ok: boolean; error?: string; entry?: MemoryEntry } { const result = this.write({ ...replacement, supersedes: oldId }); if (!result.ok || !result.entry) return result; if (this.db) { const old = this.db.prepare('SELECT id FROM memories WHERE id=?').get(oldId); if (!old) return { ok: false, error: 'memory to supersede not found' }; this.db.prepare('UPDATE memories SET contradicted_by=?, updated_at=? WHERE id=?').run(result.entry.id, now(), oldId); } else { const old = this.fallback.memories.find((entry) => entry.id === oldId); if (!old) return { ok: false, error: 'memory to supersede not found' }; old.contradictedBy = result.entry.id; old.updatedAt = now(); this.saveFallback(); } return result; }
